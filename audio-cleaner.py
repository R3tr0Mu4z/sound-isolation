...
# Credit : https://github.com/JusperLee/Looking-to-Listen-at-the-Cocktail-Party/blob/master/data/audio_data/audio_norm.py
# Normalize audio
...
for f in files:
    ...
    audio, _ = librosa.load('./audios/'+f, sr=16000)
    max = np.max(np.abs(audio))
    norm_audio = np.divide(audio, max)


    reduced_noise = nr.reduce_noise(y=norm_audio, sr=16000)
    np.save("./audios-clean/"+f, fast_stft(reduced_noise))
    wavfile.write("./audios-clean/"+f, 16000, reduced_noise)

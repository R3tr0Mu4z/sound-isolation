...
noise, sr = librosa.load('./noise.wav', sr=16000)


max = np.max(np.abs(noise))
noise = np.divide(noise, max*4)
...
for f in files:
    signal, sr = librosa.load('./audios-clean/'+f, sr=16000)

    if(len(noise)>len(signal)):
        noise=noise[0:len(signal)]
    
    signal = signal + noise
    signal = fast_stft(signal)
    np.save("./audios-noisy/"+f, signal)